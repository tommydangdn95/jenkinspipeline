...
def buildSource(){
    ...
    withEnv(["PATH+NODE=${tool name: 'nodejs 11.14.0', type: 'jenkins.plugins.nodejs.tools.NodeJSInstallation'}/bin"]) {
        ...
        stage('Copy Env file') {
            ...
            // get data from env resource file
            def data = getEnvFile()
        ...
        stage("Copy build source to tmp folder") {
            ...
            sh "cp -rf ${build_package}/* ${tmp_buildSource}"
        }
    }
}
...
def deploySource(){
    ...
    // deploy folder
    def deploy_folder = CONFIG["${params.PROFILE}"]["deploy-folder"]["${job_name}"] + "${params.PROFILE}/" + CONFIG["${params.PROFILE}"]["job"]["${job_name}"]["deploy-folder-suffix"] + "." + timeCreateFolder + "-${BUILD_NUMBER}"
    ...
    withCredentials([sshUserPrivateKey(credentialsId: "${credentialsIdSv}", keyFileVariable: 'identity', usernameVariable: 'userName')]) {
        ...
        stage("Copy all file and folder and file to current folder") {
            ...
            for (item in list_server) {
                ...
                copy_file_tasks["Cp file to main dir in ${itemServer.name}"] = {
                    sshCommand remote: itemServer, command: "rm -rf ${current_folder}/*"
        ...
        stage("Resart server nginx") {
            FAILED_STAGE = "${STAGE_NAME}"
            for (item in list_server) {
                def itemServer = item
                if(Parameters.instance.APP_ENV == "dev-prod") {
                    if(item.host == "192.168.1.220") { 
                        restart_server_tasks["Restart Pod nginx"] = {
            ...
            // parallel copy folder each server
            parallel restart_server_tasks
        }

        stage('Create Build ID Log And Show Change Log') {
...
def getEnvFile() {
    def path_resource = Parameters.instance.PATH_SYS_ENV_FE + "/.env.${params.PROFILE}"
    def envFile =  libraryResource "${path_resource}"
    return envFile
}