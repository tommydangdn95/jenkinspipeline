...
def buildSource(){
    ...
    def username = CONFIG["${params.PROFILE}"]["job"]["${job_name}"]["user-db"]
    def password = CONFIG["${params.PROFILE}"]["job"]["${job_name}"]["password-db"]
...
def readJsonConfigFile() {
    ...
    def path_config = Parameters.instance.PATH_SYS_CONFIG_STAG_LIVE
...
def getApplicationFileReousrce() {
    def path_resource = "vn/tcx/application/" + Parameters.instance.jobName + "/application-${params.PROFILE}.yml"
