...
def buildSource(){
    ...
    def job_name = Parameters.instance.jobName
    def renamed_package = "${job_name}.war"

    // folder package build out
    def build_package = CONFIG["${params.PROFILE}"]["job"]["${job_name}"]["send-file-folder"]

    // origin package name
    def original_package = CONFIG["${params.PROFILE}"]["job"]["${job_name}"]["original-name-package"]

    // creadential
    def credentialsIdBbk = CONFIG["${params.PROFILE}"]["credentialsId"]
    
    // url git
    def gitUrl = CONFIG["${params.PROFILE}"]["job"]["${job_name}"]["git-url"]
            ...
            parallel 'Show Status Database': {
                stage('Show Status Database') {
                    if (params.STATUS_DB == true) {
                        FAILED_STAGE = "${STAGE_NAME}"
                        sh "mvn liquibase:status -P${params.PROFILE}"
                    }
                    else{
            ...
            'Update Database': {
                stage('Update Database') {
                    if (params.UPDATE_DB == true) {
                        FAILED_STAGE = "${STAGE_NAME}"
                        sh "mvn liquibase:update -P${params.PROFILE}"
                    }
                    else{
                        Utils.markStageSkippedForConditional("${STAGE_NAME}")
                        echo "Skip stage"
            ...
            'Drop Database': {
                stage('Drop Database') {
                    if (params.DROP_DB == true) {
                        FAILED_STAGE = "${STAGE_NAME}"
                        sh "mvn liquibase:dropAll -P${params.PROFILE}"
                    }
                    else{
                        Utils.markStageSkippedForConditional("${STAGE_NAME}")
                        echo "Skip stage"
...
def deploySource(){
    ...
    def job_name = Parameters.instance.jobName
    def renamed_package = "${job_name}.war"
    def common_name = Parameters.instance.commonName
    def group_servers = Parameters.instance.groupServers
    ...
    // current running app
    def current_folder = CONFIG["${params.PROFILE}"]["job"]["${job_name}"]["running-folder"]

    // folder package build out
    def tmp_build_package = getTmpBuildSourceFolder()

    // restart command
    def restart_command = CONFIG["${params.PROFILE}"]["job"]["${job_name}"]["command-restart"]
    ...
    withCredentials([sshUserPrivateKey(credentialsId: "${credentialsIdSv}", keyFileVariable: 'identity', usernameVariable: 'userName')]) {
        ...
        def rawListServer = CONFIG["${params.PROFILE}"]["servers"]["groups"]["${group_servers}"].toString()
...
def readJsonConfigFile() {
    ...
    def path_config = Parameters.instance.PATH_SYS_CONFIG_DEV
